"""Train dense per-patch depth probes on cached Experiment 1 patch features."""
...
__all__ = [
    ...
    "evaluate_dense_depth_arrays",
]


@dataclass
class DenseDepthTrainConfig:
    """Hyperparameters for dense-depth linear probe training."""

    task: str = "dense_depth_patches"
    epochs: int = 40
    batch_size: int = 256
    lr: float = 1e-3
    ...
    monitor: str = "val_ssi_l1_mean"
    feature_mode: str = "patch"
    target_mode: str = "ssi_depth"
...
def evaluate_dense_depth_arrays(
    ...
    *,
    ...
    """Run dense-depth inference and compute monocular depth metrics.

    ``max_samples_for_metrics`` subsamples the metric computation (not the
    prediction tensor) for cheap per-epoch monitoring. The returned prediction
    tensor still covers every input row.
    """
    ...
    metrics = dense_depth_metrics(out_for_metrics, tgt_for_metrics, msk_for_metrics)
    if return_per_sample:
        sample_metrics = dense_depth_sample_metrics(
            output_tensor,
            target_tensor,
            mask_tensor,
        )
...
def _make_scheduler(
    ...
    *,
    cfg: DenseDepthTrainConfig,
    steps_per_epoch: int,
) -> Optional[LambdaLR]:
    if str(cfg.scheduler).lower() in {"", "none", "constant"}:
        return None
    if str(cfg.scheduler).lower() != "cosine_warmup":
        raise ValueError(f"Unsupported dense-depth scheduler: {cfg.scheduler}")
...
def _monitor_value(row: Mapping[str, float], *, cfg: DenseDepthTrainConfig) -> float:
...
def _split_arrays(
    ...
    *,
    ...
    dataset = Exp1DenseDepthDataset(
        ...
        depth_statistic=str(cfg.depth_statistic),
...
def train_dense_depth_probe(
    *,
    ...
    config: Optional[DenseDepthTrainConfig] = None,
) -> dict[str, Any]:
    """Train and evaluate one dense-depth probe."""
    cfg = copy.deepcopy(config) if config is not None else DenseDepthTrainConfig()
    ...
    for epoch in range(1, int(cfg.epochs) + 1):
        ...
        for xb, yb, mb in train_loader:
            ...
            loss = ssi_l1_loss(pred, yb.to(device), mb.to(device))
            loss.backward()
            optimizer.step()
        ...
        _, train_metrics = evaluate_dense_depth_arrays(
            model,
        ...
        if val_arrays is not None:
            _, val_metrics = evaluate_dense_depth_arrays(
                model,
    ...
    for split_name, arrays in split_arrays.items():
        predictions, split_metrics, sample_metrics = evaluate_dense_depth_arrays(
            model,
        ...
        prediction_paths[split_name] = _save_predictions(
            output_dir, split_name, arrays, predictions
        )
    ...
    metadata = {
        ...
        "monitor": cfg.monitor,
        "depth_statistic": cfg.depth_statistic,
        "min_valid_fraction_per_patch": float(cfg.min_valid_fraction_per_patch),
