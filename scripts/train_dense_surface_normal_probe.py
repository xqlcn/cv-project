#!/usr/bin/env python3
"""Train one dense per-patch surface-normal probe for Experiment 1."""
...
from exp1.features.storage import patch_feature_cache_path  # noqa: E402
from exp1.probes.train_dense_surface_normals import (  # noqa: E402
...
def _dense_cfg_section(cfg) -> dict:
    dense_cfg = cfg.probe.get("dense_surface_normal")
    if dense_cfg is None:
        dense_cfg = OmegaConf.create({})
    return dense_cfg


def _build_train_config(cfg, args: argparse.Namespace) -> DenseSurfaceNormalTrainConfig:
    dense = _dense_cfg_section(cfg)
    early = cfg.probe.get("early_stopping", {})
    return DenseSurfaceNormalTrainConfig(
        task=str(dense.get("task", "dense_surface_normal_patches")),
        epochs=int(
            args.epochs
        ...
        lr=float(args.lr if args.lr is not None else dense.get("lr", cfg.probe.lr)),
        weight_decay=float(
            args.weight_decay
        ...
        monitor=str(dense.get("monitor", "val_angular_error_deg_mean")),
        feature_mode=str(dense.get("feature_mode", "patch")),
        target_mode=str(dense.get("target_mode", "camera_frame_normals")),
...
def main() -> None:
    ...
    layer_default = str((cfg.models.get("dense_layers") or cfg.models.layers)[0])
