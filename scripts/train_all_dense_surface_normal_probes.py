#!/usr/bin/env python3
"""Train all configured dense per-patch surface-normal probes for Experiment 1."""
...
from train_dense_surface_normal_probe import _build_train_config  # noqa: E402
...
def _texture_jobs(cfg, explicit_texture_condition):
    ...
    dense_cfg = cfg.probe.get("dense_surface_normal", {}) or {}
    dense_eval_cfg = dense_cfg.get("evaluation", {}) or {}
    within_texture = bool(
        dense_eval_cfg.get(
            "within_texture",
            base_eval_cfg.get("within_texture", False),
        )
    )
    cross_texture = bool(
        dense_eval_cfg.get(
            "cross_texture",
            base_eval_cfg.get("cross_texture", False),
        )
    )
    jobs = []
    if within_texture:
...
def main() -> None:
    ...
    for model_name in models:
        for layer_name in layers:
            ...
            for texture_job in texture_jobs:
                tag = (
                    f"{model_name}/{layer_name}/dense_surface_normal_patches/"
                    f"{texture_job['name']}"
                )
                output_dir = (
                    Path(base_output)
    ...
    print(f"Completed {len(completed)} dense-normal probe(s); skipped {len(skipped)}.")
